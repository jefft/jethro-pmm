...
`getSmsProvider()` constructs the chain on every call — constants cannot
change within a request, so all callers receive an identical chain (though the
token resolver's person-row cache is per-invocation).
