...
class SmsBroadcastSmsProvider implements SmsProvider
{
	 ...
	 * @param string|null $baseUrl  Absolute base URL; bare-path constants are resolved against this.
	...
	public static function fromConstants(bool $tfa = false, ?string $baseUrl = null): static
	{
		...
		if ($url === '') {
			$url = rtrim((string) ifdef('SMS_SMSBROADCAST_URL', self::CANONICAL_URL), '/');
		}
