...
const ADMIN_EMAIL = "demo@example.org";
...
test.describe("Setup wizard", () => {
  ...
  test("installs Jethro into the empty jethro_functest_walkthrough database", async ({ page }) => {
    // Form fill + schema install + login + admin page + 3 SQL checks in one
    // test: too much for the default 30s when 8 workers share one php-fpm
    // pool, so allow 3x (Playwright slow()).
    test.slow();
...
test.describe("Jethro actions", () => {
  ...
  test("adds the reference families and their members", async ({ page }) => {
    const families = [
      {
        name: "Calvin",
        members: [
          { first: "John", last: "Calvin", gender: "male", congregation: "6pm" },
          { first: "Idelette", last: "de Bure", gender: "female", congregation: "4pm" },
        ],
      },
      {
        name: "Luther",
        members: [
          { first: "Martin", last: "Luther", gender: "male", congregation: "4pm" },
          { first: "Katharina", last: "von Bora", gender: "female", congregation: "4pm" },
        ],
      },
    ];
    for (const fam of families) {
      ...
      for (let i = 0; i < fam.members.length; i++) {
        ...
        await page.selectOption(`select[name="${p}congregationid"]`, { label: m.congregation });
  ...
  test("creates and views a person query (report)", async ({ page }) => {
    ...
    // Enable the "Date of Birth" custom-field filter. Its default criterion
    // ("filled in with any value") matches exactly the one person given a
    // birth date earlier in the walkthrough: John Calvin.
    await page.click("#enable_custom_1");
    // Surface the Congregation column (replacing the default "view details"
    // link, the fifth "show me" row) so the bulk update below is verifiable.
    await page
      .locator(
        "#body > form > table.table-condensed.expandable.indent-left > tbody > tr:nth-child(5) > td:nth-child(2) > select"
      )
      .selectOption("p.congregationid");
    await page.click('input[value="Save and view results"]');

    // The report redirects to its results view; first/last names render in
    // separate cells, so assert the single row contains both.
    const resultRows = page.locator("table.query-results tbody tr");
    await expect(resultRows).toHaveCount(1);
    await expect(resultRows.first()).toContainText("John");
    await expect(resultRows.first()).toContainText("Calvin");
    await expect(resultRows.first()).toContainText("6pm"); // starting congregation

    // Bulk-update: select the row, choose "Set field(s)", move the person from
    // "6pm" to "4pm", accept the native confirmation, and submit.
    await page
      .locator("#body > form > table > tbody > tr > td.selector.no-print.narrow > input")
      .check();
    await page.selectOption("#bulk-action-chooser", "update-field");
    await page.selectOption('#update-field select[name="congregationid"]', { label: "4pm" });
    page.once("dialog", (dialog) => dialog.accept());
    await page.click('#update-field input[value="Go"]');

    // The update redirects back to the report, now showing the new congregation.
    await expect(page.getByText("Fields updated for 1 persons")).toBeVisible();
    await expect(resultRows).toHaveCount(1);
    await expect(resultRows.first()).toContainText("4pm");
  });
