...
class Call_DTMF extends Call
{
	...
	{
		// samples per second
		$sample_rate = isset($sample_rate) ? (int) $sample_rate : 8000;

		// signal length in milliseconds
		$signal_length = isset($signal_length) ? (int) $signal_length : 100;

		// break between signals in milliseconds
		$break_length = isset($break_length) ? (int) $break_length : 100;

		// pause length in milliseconds - pause character is ','
		$pause_length = isset($pause_length) ? (int) $pause_length : 500;

		// amplitude of wave file in the range 0-64
		$amplitude = isset($amplitude) ? (int) $amplitude : 64;
