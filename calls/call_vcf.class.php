...
class Call_vcf extends Call
{
	function run()
	{
		...
		$merge_data = $GLOBALS['system']->getDBObjectData('person', ['id' => $_REQUEST['personid']]);
		$dummy = new Person();
		$family = new Family();
		...
		foreach ($merge_data as $id => $row) {
			...
			echo 'SOURCE:'.build_url(['personid' => [$id]])."\n"; // where to get vcard from
			echo 'URL:'.build_url(['call' => null, 'view' => 'persons', 'personid' => $id])."\n"; // definitive location for full details
			echo 'UID:'.build_url(['call' => null, 'view' => 'persons', 'personid' => $id])."\n";
			echo "END:VCARD\n\n";
		}
	}
}
