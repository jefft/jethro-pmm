...
if (count($roster_array) > 2) {
	$rds = [];
	$roster_date_index = array_search('Date', $roster_array[1], true);
...
if ($sendsms) { // make the sms message!
	...
	if (count($assignees) > 0) {
		...
		if (!empty($assignees)) {
			...
			$successes = $failures = $rawresponse = [];
