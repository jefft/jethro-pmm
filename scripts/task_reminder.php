...
if (ifdef('TASK_NOTIFICATION_ENABLED', false) == false) {
	if ($VERBOSE) {
		echo "Task notification is disabled in conf.php - exiting \n";
	}
	exit;
}
...
foreach ($reminders as $reminder) {
	...
	$message = Emailer::newMessage()
	  ->setSubject($subject)
	  ->setFrom([TASK_NOTIFICATION_FROM_ADDRESS => $fromText])
	  ->setTo([$reminder['email'] => $reminder['first_name'].' '.$reminder['last_name']])
	  ->setBody($content)
