...
class View_Attendance__Display extends View
{
	...
	private $letters = [0 => 'A', 1 => 'P', '' => '?', '-1' => ''];
	private $classes = [0 => 'absent', 1 => 'present', '' => 'unknown', '-1' => ''];
	...
	function processView()
	{
		if (empty($_REQUEST['params_submitted'])) {
			if (!empty($_SESSION['attendance'])) {
				$this->age_brackets = array_get($_SESSION['attendance'], 'age_brackets', []);
				$this->statuses = array_get($_SESSION['attendance'], 'statuses', []);
			...
			$this->age_brackets = $_SESSION['attendance']['age_brackets'] = array_get($_REQUEST, 'age_brackets', []);
			$this->statuses = $_SESSION['attendance']['statuses'] = array_get($_REQUEST, 'statuses', []);
			...
			$this->start_date = $_SESSION['attendance']['start_date'] = process_widget('start_date', ['type' => 'date']);
			$this->end_date = $_SESSION['attendance']['end_date'] = process_widget('end_date', ['type' => 'date']);
		...
		for ($i = count($this->cohortids); $i >= 0; --$i) {
			if (empty($this->cohortids[$i])) {
				unset($this->cohortids[$i]);
			}
		}
	}
	...
	private function _printParams()
	{
		...
		?>
				...
				<tr>
					...
					<td colspan="2">
						<?php print_widget('start_date', ['type' => 'date'], $this->start_date); ?>
					</td>
				</tr>
				<tr>
					<th><?php echo _('And'); ?></th>
					<td colspan="2">
						<?php print_widget('end_date', ['type' => 'date'], $this->end_date); ?>
					</td>
				</tr>
				<tr>
					<th>Order&nbsp;by</th>
					<td>
						<?php print_widget('order', ['type' => 'select', 'options' => Attendance_Record_Set::getOrderOptions()], $this->order); ?>
					</td>
				</tr>
				<tr>
	...
	private function _printResultSet($cohortid)
	{
		...
		if ($this->age_brackets) {
			$params['(age_bracketid'] = $this->age_brackets;
		}
		if ($this->statuses) {
			$params['(status'] = $this->statuses;
		}
		...
		[$dates, $attendances, $totals] = Attendance_Record_Set::getAttendances((array) $congid, $groupid, $params, $this->start_date, $this->end_date, $this->order);
		if (empty($dates)) {
			?>
		...
		$headcounts = Headcount::fetchRange($congid ? 'congregation' : 'person_group', $congid ?: $groupid, $this->start_date, $this->end_date);
		$dummy = new Person();
		?>
		...
		?>
			...
			foreach ($attendances as $personid => $record) {
				$class = in_array($record['status'], Person_Status::getArchivedIDs(), true) ? 'class="archived"' : '';
				?>
	...
	private function _printResultsTabular()
	{
		...
		$all_dates = $all_attendances = $all_totals = $all_headcounts = [];
		if (!empty($this->cohortids)) {
			foreach ($this->cohortids as $cohortid) {
				$congid = $groupid = null;
				[$type, $id] = explode('-', $cohortid);
				...
				[$cdates, $cattendances, $ctotals] = Attendance_Record_Set::getAttendances((array) $congid, $groupid, $params, $this->start_date, $this->end_date);
				if (empty($params)) {
					// Headcounts and extras don't make sense when we are only viewing a segment of the total attendance
					$hc = Headcount::fetchRange($congid ? 'congregation' : 'person_group', $congid ?: $groupid, $this->start_date, $this->end_date);
				...
				foreach ($cattendances as $personid => $cat) {
					...
					foreach ($cat as $k => $v) {
						if (!in_array($k, ['first_name', 'last_name', 'membership_status', 'status'], true)) {
		...
		foreach ($all_persons as $personid => $details) {
			...
			$letters = '';
			$class = in_array($details['status'], Person_Status::getArchivedIDs(), true) ? 'class="archived"' : '';
			?>
				<tr <?php echo $class; ?>>
					<td class="nowrap">
			...
			foreach ($all_dates as $date) {
				...
				if ($this->format == 'totals') {
					$score = '';
					foreach ($this->cohortids as $cohortid) {
						$catt = array_get($all_attendances[$personid], $cohortid, []);
						$x = array_get($catt, $date, '');
						if (strlen($x)) {
							$score = (int) $score + rtrim($x, '*');
						} // The '*' suffix indicates a planned absence
					}
					...
					foreach ($this->cohortids as $cohortid) {
						if (!in_array($cohortid, array_get($all_persons[$personid], 'cohortids', []), true)) {
							$class = 'disabled';
							$letter = '';
							$pa = false;
						} else {
		...
		if ($this->format != 'totals') { // headcounts don't make sense when we collapse groups down into totals
			...
			if (empty($params)) {
				...
				foreach ($all_dates as $date) {
					$hc = array_get($all_headcounts, $date, []);
					foreach ($this->cohortids as $cohortid) {
			...
			foreach ($all_dates as $date) {
				$tots = array_get($all_totals, $date, []);
				foreach ($this->cohortids as $cohortid) {
					echo '<td>'.array_get(array_get($tots, $cohortid, []), 1, 0).'<input type="hidden" name="present[]" value="'.array_get(array_get($tots, $cohortid, []), 1, 0).'"></td>';
				}
			}
			?>
					<td colspan="2">&nbsp;</td>
			...
			foreach ($all_dates as $date) {
				$tots = array_get($all_totals, $date, []);
				foreach ($this->cohortids as $cohortid) {
					echo '<td>'.array_get(array_get($tots, $cohortid, []), 0, 0).'<input type="hidden" name="absent[]" value="'.array_get(array_get($tots, $cohortid, []), 0, 0).'"></td>';
				}
			}
			?>
					<td colspan="2">&nbsp;</td>
			...
			if (empty($params)) {
				...
				foreach ($all_dates as $date) {
					...
					$hc = array_get($all_headcounts, $date, []);
					foreach ($this->cohortids as $cohortid) {
						$present = array_get(array_get($tots, $cohortid, []), 1, 0);
