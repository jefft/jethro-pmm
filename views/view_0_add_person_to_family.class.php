...
class View__Add_Person_To_Family extends View
{
			...
			if ($this->_person->create()) {
				if (!empty($_POST['execute_plan'])) {
					foreach ($_POST['execute_plan'] as $planid) {
						...
						$plan->execute('person', $this->_person->id, process_widget('plan_reference_date', ['type' => 'date']));
					}
				}

				$GLOBALS['system']->doTransaction('commit');
				add_message('New family member added');
				redirect('families', ['familyid' => $this->_family->id]); // exits
			} else {
	...
	function printView()
	{
		...
		?>
			...
			<a href="<?php echo build_url(['view' => 'families']); ?>" class="btn">Cancel</a>
		</form>
		<?php
	}
}
