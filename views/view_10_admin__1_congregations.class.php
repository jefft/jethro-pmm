...
class View_Admin__Congregations extends View
{
	...
	function printView()
	{
		...
		<table class="table table-hover table-auto-width">
			...
			<tbody>
			<?php
			$congs = $GLOBALS['system']->getDBObjectData('congregation', [], 'OR', 'meeting_time');
		$deletePrinted = false;
		...
		foreach ($congs as $id => $cong) {
			...
			?>
					...
					if ($congObject->canDelete()) {
						?>
						<a href="<?php echo build_url(['action' => 'delete', 'congregationid' => $id]); ?>" data-method="post"><i class="icon-trash"></i><?php echo _('Delete'); ?></a>
						<?php
						$deletePrinted = true;
					}
			?>
