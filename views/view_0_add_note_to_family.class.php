	...
	function processView()
	{
		...
		if (array_get($_REQUEST, 'new_note_submitted')) {
			...
			if ($this->_note->create()) {
				...
				redirect('families', ['familyid' => $this->_family->id], 'note_'.$this->_note->id); // exits
			}
		} else {
			$members = $this->_family->getMemberData();
			if (count($members) == 1) {
				add_message(_('Family has only one member, so adding note to that person instead'));
				redirect('_add_note_to_person', ['personid' => key($members)]); // exits
			}
		}
	}
	...
	function printView()
	{
		...
		?>
			<div class="controls">
				...
				<a class="btn" href="<?php echo build_url(['view' => 'families', 'familyid' => $this->_family->id]); ?>"><?php echo _('Cancel'); ?></a>
		</form>
		<?php
