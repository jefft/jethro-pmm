...
class View_Families__Add extends View
{
	...
	function processView()
	{
		...
		if (array_get($_REQUEST, 'new_family_submitted')) {
			...
			if ($success) {
				...
				if (!empty($_POST['execute_plan'])) {
					foreach ($_POST['execute_plan'] as $planid) {
						...
						$plan->execute('family', $this->_family->id, process_widget('plan_reference_date', ['type' => 'date']));
					}
				}
			}
			...
			if ($success) {
				...
				if (count($members) > 1) {
					redirect('families', ['familyid' => $this->_family->id]);
				} else {
					redirect('persons', ['personid' => $members[0]->id]);
				}
			} else {
				$GLOBALS['system']->doTransaction('rollback');
				$this->_family->id = 0;
	...
	function printSimilarFamilies()
	{
		...
		foreach ($this->_similar_families as $family) {
			?>
			<h4><a href="<?php echo build_url(['view' => 'families', 'familyid' => $family->id]); ?>"><?php echo _('Family'); ?>#<?php echo $family->id; ?></a></h4>
			<?php
	...
	function printForm()
	{
		...
		$customFields = $GLOBALS['system']->getDBObjectData('custom_field', ['show_add_family' => 1], 'AND', 'rank');
		?>
		...
		$group_options = $GLOBALS['system']->getDBObjectData('person_group', ['!show_add_family' => 'no', 'is_archived' => 0], 'AND');
		if (SizeDetector::isNarrow() || count($customFields) > 0 || count($group_options) > 0) {
			...
			foreach ($customFields as $fieldID => $fDetails) {
				...
				?>
							...
							<div class="fullwidth"><?php $field->printWidget('', [], 'members_0_'); ?></div>
							<?php
			}
			?>
		...
		?>
			...
			<?php
		$this->_family->printForm('', ['home_tel', 'address_street', 'address_suburb', 'address_state', 'address_postcode']);
		?>
		...
		if ($plan_chooser = Action_Plan::getMultiChooser('execute_plan', 'create_family')) {
			...
			<div class="indent-left">
				...
				<p><?php _('Reference date for plans:'); ?> <?php print_widget('plan_reference_date', ['type' => 'date'], null); ?></p>
			</div>
			<?php
		}
		?>
