...
class View_Admin__Action_Plans extends View
{
	...
	function processView()
	{
		...
		if ($this->_plan && !empty($_REQUEST['delete'])) {
		...
		} elseif (!empty($_POST['plan_submitted'])) {
			...
			$this->_plan = null;
		} elseif ($this->_plan && $this->_plan->id) {
			if (!$this->_plan->acquireLock()) {
		...
		if (!empty($this->_plan)) {
			if (empty($this->_plan->id) || $this->_plan->haveLock()) {
				...
				?>
					...
					<a class="btn" href="<?php echo build_url(['planid' => null]); ?>">Cancel</a>
				</form>
				<?php
			} else {
			...
			<p><a href="<?php echo build_url(['planid' => 0, 'delete' => null]); ?>"><i class="icon-plus-sign"></i>Add new plan</a></p>
			<?php
			$saved_plans = $GLOBALS['system']->getDBObjectData('action_plan', [], '', 'name');
