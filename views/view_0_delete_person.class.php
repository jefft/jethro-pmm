...
';
	...
	function processView()
	{
		...
		$this->_notes = $GLOBALS['system']->getDBObjectData(
			'person_note',
			['personid' => $this->_person->id, 'status' => 'pending'],
			'AND',
		);
		...
		if (count($members) == 1) {
			$fnotes = $GLOBALS['system']->getDBObjectData(
				'family_note',
				['familyid' => $family->id, 'status' => 'pending'],
				'AND',
			);
		...
		if (empty($this->_staff_member) && !empty($_POST['confirm_delete'])) {
			...
			redirect('home');
		} elseif (!empty($_POST['confirm_archiveclean'])) {
			...
			if ($res == 2) {
				$message .= ' and so has their family';
			}
			if ($res == 1) {
				$message .= ' but their family has a remaining member and has been left untouched';
			}
			if ($res) {
				add_message($message, 'success');
				redirect('persons', ['personid' => $this->_person->id]); // exits
			}
		} elseif (!empty($_POST['confirm_archive'])) {
			if (!$this->_person->acquireLock()) {
				add_message('This person cannot be archived because somebody else holds the lock.  Try again later.', 'error');
				redirect('persons', ['personid' => $this->_person->id]); // exits
			}
	...
	public function printView()
	{
		...
		} elseif (Roster_Role_Assignment::hasAssignments($this->_person->id) || $this->_person->hasAttendance()) {
			?>
