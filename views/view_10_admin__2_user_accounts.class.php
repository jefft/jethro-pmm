...
class View_Admin__User_Accounts extends View
{
	...
	function printView()
	{
		...
		if (empty($_REQUEST['show_inactive'])) {
			?>
			<a class="pull-right hidden-phone" href="<?php echo build_url(['show_inactive' => 1]); ?>"><i class="icon-eye-open"></i><?php echo _('Include inactive accounts'); ?></a>
			<?php
		} else {
			?>
			<a class="pull-right hidden-phone" href="<?php echo build_url(['show_inactive' => 0]); ?>"><i class="icon-eye-close"></i><?php echo _('Hide inactive accounts'); ?></a>
			<?php
		}
		?>
		...
		<table class="table table-striped table-hover table-min-width">
			...
			<tbody>
		<?php
		$conds = empty($_REQUEST['show_inactive']) ? ['active' => 1] : [];
	...
	private function _printPermissions($level)
	{
		...
		if (null === $PERM_LEVELS) {
			include 'include/permission_levels.php';
		}
