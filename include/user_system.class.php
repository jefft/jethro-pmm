...
require_once __DIR__.'/abstract_user_system.class.php';
...
class User_System extends Abstract_User_System
{
	...
	public function __construct()
	{
		...
		if (!empty($_REQUEST['logout'])) {
			$this->_logOut();
		} elseif (empty($_SESSION['user']) && !empty($_POST['username'])) {
		...
		} elseif (!empty($_SESSION['2fa']['pending_user'])) {
			...
			if ($_SESSION['2fa']['expiry'] < time()) {
				...
				return;
			} elseif (array_get($_POST, '2fa_key') != $_SESSION['2fa']['key']) {
			...
			if (!empty($_POST['2fa_code'])) {
				if ($_POST['2fa_code'] == $_SESSION['2fa']['code']) {
					...
					if ($_SESSION['2fa']['wrong_code_count'] > 1) {
						$this->_reset2FA();
					} // start again from scratch after 2 wrong codes.

					return;
				}
			}
		} elseif (!empty($_SESSION['user_requiring_password_upgrade'])) {
			...
			if ($_SESSION['password_upgrade_expiry'] < time()) {
				...
				return;
			} elseif (array_get($_POST, 'password_upgrade_key') != $_SESSION['password_upgrade_key']) {
				unset($_SESSION['user_requiring_password_upgrade']);
	...
	private function _logUserIn($user_details)
	{
		...
		session_write_close();
		header('Location: '.build_url([])); // the login form was POSTed; we redirect so the subsequent page load is a clean GET request.
		exit;
	}
	...
	{
		$res = [];
		if (!empty($_SESSION['user']['group_restrictions'])) {
			$res['group'] = $_SESSION['user']['group_restrictions'];
		}
		if (!empty($_SESSION['user']['congregation_restrictions'])) {
			$res['congregation'] = $_SESSION['user']['congregation_restrictions'];
		}
		if ($type) {
			$res = array_get($res, $type);
		}
	...
	public function havePerm($permission)
	{
		if (!is_int($permission)) {
			throw new RuntimeException('Non-numeric permission level is invalid');
		}
		if ($permission == 0) {
			return true;
		}
		...
		if (!array_key_exists($permission, $this->_permission_levels)) {
			return false;
		} // disabled feature
	...
	private function _findUser($username, $password)
	{
		...
		if (!empty($row) && jethro_password_verify($password, $row['password'])) {
			$row['congregation_restrictions'] = empty($row['congregation_restrictions']) ? [] : explode(',', $row['congregation_restrictions']);
			$row['group_restrictions'] = empty($row['group_restrictions']) ? [] : explode(',', $row['group_restrictions']);
	 ...
	 *
	 * @return bool TRUE if this person needs to do 2-factor auth before being logged in
	 */
	private function _require2FA($user_details)
	{
		...
		if (defined('2FA_ENABLED') && constant('2FA_ENABLED') == false) {
			return false;
		}
	...
	public function handle2FAMobileTelChange($person, $old_mobile)
	{
		...
		if ($staff_member->requires2FA()) {
			...
			$this->_send2FAMessage($msg, ['mobile_tel' => $old_mobile]);
		}
	}
	...
	private function _send2FAMessage($msg, $recipient)
	{
		...
		$res = SMS_Sender::sendMessage($msg, [$recipient]);
	...
	private function _process2FATrust()
	{
		...
		if (!empty($_REQUEST['2fa_trust'])) {
			...
			$res = $db->exec($SQL);
			if (!$res) {
				$this->_2faLog('ERROR: Failed saving trust record to DB');
			}

			$expiry = strtotime('+'.$trust_days.' days');
			$res = setcookie('Jethro2FATrust', $trust_token, $expiry);
			if (!$res) {
				trigger_error('Could not save trust cookie');
			}
	...
	private function _2faLog($message)
	{
		...
		if (strtolower(ifdef('2FA_LOG_LEVEL', 'full')) == 'quiet') {
			if (!str_starts_with($message, 'ERROR')) {
				return;
			} // in 'quiet' mode, only log messages starting with ERROR.
		}

		$message = date('c').' '.$message;
		$res = @error_log($message."\n", 3, $logfile);
		if (!$res) {
			trigger_error('Failed writing to 2fa logfile', \E_USER_WARNING);
		}
	}
	...
	private function _notifySysAdmins($message)
	{
		...
		if (strlen($from_address)) {
			$message->setFrom([$from_address => SYSTEM_NAME.' Jethro System']);
		} else {
			$message->setFrom([reset($emails) => SYSTEM_NAME.' Jethro System']);
		}
