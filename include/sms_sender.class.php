...
class SMS_Sender
{
	...
	public static function getRecipients()
	{
		...
		switch ($smstype) {
			case 'family':
				...
				$recips = $GLOBALS['system']->getDBObjectData('person', ['(age_bracketid' => Age_Bracket::getAdults(), '(familyid' => array_keys($families), '!mobile_tel' => '', '!(status' => Person_Status::getArchivedIDs()], 'AND');
				$blanks = $GLOBALS['system']->getDBObjectData('person', ['(age_bracketid' => Age_Bracket::getAdults(), '(familyid' => array_keys($families), 'mobile_tel' => '', '!(status' => Person_Status::getArchivedIDs()], 'AND');
			...
			case 'person':
			default:
				$recips = $GLOBALS['system']->getDBObjectData('person', ['id' => $personids, '!mobile_tel' => '', '!(status' => Person_Status::getArchivedIDs()], 'AND');
	 ...
	 * @param bool   $saveAsNote Whether to save a note against the recipients
	 *
	 * @return array(
	...
	public static function sendMessage($message, $recips, $saveAsNote = false)
	{
		...
		if (str_contains($content, '_USER_MOBILE_')) {
			if (ifdef('OVERRIDE_USER_MOBILE')) {
				...
				if (!strlen($me->getValue('mobile_tel'))) {
					return ['executed' => false, 'successes' => [], 'failures' => [], 'rawresponse' => '',
		...
		if (str_contains($content, '_USER_EMAIL_')) {
			$me = $GLOBALS['system']->getDBObject('person', $GLOBALS['user_system']->getCurrentUser('id'));
			if (empty($me)) {
		...
		if (strlen(ifdef('SMS_LOCAL_PREFIX')) && strlen(ifdef('SMS_INTERNATIONAL_PREFIX')) && str_contains(self::_getSetting('HEADER_TEMPLATE'), '_RECIPIENTS_INTERNATIONAL')
		) {
			$intls = [];
		...
		try {
			...
			if (!$fp) {
				$http_error = 'ERROR: Unable to connect to SMS Server.<br>'.implode('<br>', $http_response_header);

				return ['executed' => false, 'successes' => [], 'failures' => [], 'rawresponse' => $http_error, 'error' => $http_error];
			} else {
			...
			return ['executed' => false, 'successes' => [], 'failures' => [], 'rawresponse' => $error, 'error' => $error];
		}
		...
		if ($executed) {
			...
			if ($okReg = self::_getSetting('RESPONSE_OK_REGEX')) {
				...
				if ($saveAsNote) {
					self::saveAsNote($successes, $message);
				}
			} else {
				self::logSuccess(count($mobile_tels), $message);
				if ($saveAsNote) {
					self::saveAsNote($recips, $message);
				}
			}
		}
	...
	private static function internationaliseNumber($number)
	{
		if (strlen(ifdef('SMS_LOCAL_PREFIX'))
				...
				&& str_starts_with($number, (string) SMS_LOCAL_PREFIX) // convert to string in case it is just a number, eg 0
		) {
	...
	{
		if (self::$configPrefix !== self::DEFAULT_CONFIG_PREFIX) {
			return;
		} // Log doesn't apply when using dedicated 2FA settings.
		...
		if (defined('SMS_SEND_LOGFILE') && ($file = constant('SMS_SEND_LOGFILE'))) {
			...
			if (filesize(SMS_SEND_LOGFILE) < 3) {
				// Write a header row
				$headers = ['Timestamp', 'Username', 'RecipientCount', 'MessageLength', 'Content'];
			...
			$vals = [date('c'), $username, $recip_count, strlen($message), $msg_trunc];
