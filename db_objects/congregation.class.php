...
class Congregation extends db_object
{
	...
	public function printFieldInterface($name, $prefix = '')
	{
		switch ($name) {
			case 'holds_persons':
				$params = ['type' => 'checkbox', 'attrs' => ['data-toggle' => 'visible']];
				?>
				...
				print_widget('holds_services', $params, empty($this->id) || $this->getValue('meeting_time') !== '');
				?>
	...
	public function getInstancesQueryComps($params, $logic, $order)
	{
		...
		if (!empty($restrictions['congregation'])) {
			$oldwhere = $res['where'];
			$res['where'] = 'congregation.id IN ('.implode(',', array_map([$GLOBALS['db'], 'quote'], $restrictions['congregation'])).')
							';
			if ($oldwhere) {
				$res['where'] .= ' AND ('.$oldwhere.')';
			}
		}
	...
	public static function findByName($name)
	{
		...
		static $congs = [];
		if (!isset($congs[$name])) {
			$matches = $GLOBALS['system']->getDBObjectData('congregation', ['name' => $name, 'long_name' => $name], 'OR');
			if (count($matches) == 1) {
				$congs[$name] = key($matches);
			}
		}
		if (!isset($congs[$name])) {
			trigger_error('Could not find a unique congregation with name "'.$name.'"');
		}
	...
	{
		$members = $GLOBALS['system']->getDBObjectData('person', ['congregationid' => $this->id]);
		if (count($members)) {
			if ($trigger_messages) {
		...
		$services = $GLOBALS['system']->getDBObjectData('service', ['congregationid' => $this->id]);
		if ($services) {
			if ($trigger_messages) {
				add_message('This congregation cannot be deleted because there are historical services that belong to it', 'error');
			}
