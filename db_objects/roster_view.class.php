				...
				foreach ($members_to_print as $order_num => $member_details) {
					...
					<tr>
						...
						<td class="nowrap">
							...
							<img src="<?php echo BASE_PATH; ?>/resources/img/arrow_up_thin_black.png" class="icon move-row-up" title="Move this role up" />
							<img src="<?php echo BASE_PATH; ?>/resources/img/arrow_down_thin_black.png" class="icon move-row-down" title="Move this role down" />
							<img src="<?php echo BASE_PATH; ?>/resources/img/cross_red.png" class="icon delete-row" title="Delete this role from the list" />
						</td>
					</tr>
					<?php
				}
		...
		if ($member['role_id']) {
			...
			foreach ($asn as $rank => $asn) {
				...
				if ($withLinks) {
					...
					if (('' === $asn['email'])) echo ' <img class="visible-desktop" src="'.BASE_PATH.'/resources/img/no_email.png" title="No Email Address" />';
					if (('' === $asn['mobile']) && SMS_Sender::canSend()) {
						echo ' <img class="visible-desktop" src="'.BASE_PATH.'/resources/img/no_phone.png" title="No Mobile" />';
					}
					echo '</span>';
						...
						if ($editing && empty($mdetail['readonly']) && !$haveHidden) {
							...
							foreach (array_get($ddetail['assignments'], $mdetail['role_id'], Array()) as $rank => $vs) {
								...
								if (!$public && !$vs['assigneehidden']) {
									...
									if (('' === $vs['email'])) $n .= ' <img class="visible-desktop" src="'.BASE_PATH.'/resources/img/no_email.png" title="No Email Address" />';
									if (('' === $vs['mobile']) && SMS_Sender::canSend()) {
										$n .= ' <img class="visible-desktop" src="'.BASE_PATH.'/resources/img/no_phone.png" title="No Mobile" />';
					                }
									$n .= '</span>';
									$names[] = $n;
								} else {
