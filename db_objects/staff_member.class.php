...
class Staff_Member extends Person
{
	...
	function printFieldValue($name, $value = null)
	{
		...
		if ($name == 'restrictions') {
				...
				<tr>
					<td>
						...
						if (!empty($value['congregation'])) {
							foreach ($GLOBALS['system']->getDBObjectData('congregation', ['(id' => $value['congregation']]) as $cong) {
			...
			if (!empty($value['group'])) {
				foreach ($GLOBALS['system']->getDBObjectData('person_group', ['(id' => $value['group']]) as $group) {
	...
	{
		if ($fields === null) {
			$fields = ['username', 'password', 'active', 'permissions', 'restrictions'];
		}
		$offset = array_search('permissions', array_keys($this->fields), true) + 1;
		$this->fields['restrictions'] = ['type' => 'custom'];
			...
			case 'restrictions':
				...
				if ($GLOBALS['user_system']->havePerm(PERM_SYSADMIN)) {
						...
						<tr>
							<td>
								...
								print_widget(
									...
									['type' => 'reference', 'references' => 'congregation', 'allow_multiple' => true, 'filter' => ['holds_persons' => 1]],
									array_get($this->_restrictions, 'congregation', []),
								);
					?>
							</td>
							<td>
								<?php
					Person_Group::printMultiChooser('restrictions[group]', array_get($this->_restrictions, 'group', []), [], false);
					?>
							</td>
						</tr>
					</table>
					<p class="help-inline">If you select congregations or groups here, this user will only be able to see persons who are in one of the selected congregations or one of the selected groups.  It will look like those are the only congregations, groups and persons in the system.
						Users with group <?php if (!ifdef('RESTRICTED_USERS_CAN_ADD')) {
							echo 'or congregation';
						} ?> restrictions cannot add new persons or families.  Changes to restrictions take effect immediately.
					</p>
					<?php
				} else {
	...
	function processForm($prefix = '', $fields = null)
	{
		...
		if ($GLOBALS['user_system']->havePerm(PERM_SYSADMIN) && !empty($_REQUEST['restrictions'])) {
			...
			$this->_restrictions = [];
			foreach (['congregation', 'group'] as $type) {
				$this->_restrictions[$type] = [];
				foreach (array_get($_REQUEST['restrictions'], $type, []) as $id) {
					if (!empty($id)) {
						$this->_restrictions[$type][] = $id;
					}
				}
			}
		}
	...
	function getValue($name)
	{
		if ($name == 'raw_password') {
			return array_get($this->_tmp, 'raw_password');
		}
		if ($name == 'restrictions') {
	...
	function save($update_family = true)
	{
		...
		if (!empty($GLOBALS['JETHRO_INSTALLING']) || ($GLOBALS['user_system']->getCurrentUser('id') == $this->id) || $GLOBALS['user_system']->havePerm(PERM_SYSADMIN)) {
			...
			if (isset($this->_old_restrictions)) {
				foreach (['congregation', 'group'] as $type) {
					if (array_get($this->_restrictions, $type, []) != array_get($this->_old_restrictions, $type, [])) {
	...
	{
		if (empty($this->id)) {
			throw new RuntimeException("Don't have an ID, can't insert restrictions");
		}
		foreach (['congregation', 'group'] as $type) {
	...
	{
		$others = $GLOBALS['system']->getDBObjectData('staff_member', ['username' => $this->getValue('username'), '!id' => (int) $this->id], 'AND');
		if ($others) {
