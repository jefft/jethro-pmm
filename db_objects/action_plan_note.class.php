...
class Action_Plan_Note extends Abstract_Note
{
	static function getAbstractNoteData($action_note_data, $reference_date)
	{
		...
		unset($action_note_data['action_date_type'], $action_note_data['action_date_offset'], $action_note_data['action_date_operator']);
	...
	protected static function _getFields()
	{
		...
		$res['assignee']['note'] = '';
		$res['action_date_type'] = [
			'type' => 'select',
			'options' => ['fixed' => 'fixed', 'relative' => 'relative'],
			'default' => 'relative',
			'editable' => false,
		];
		$res['action_date_offset'] = [
			'type' => 'int',
			'editable' => false,
			'default' => 0,
		];
		$res['action_date_operator'] = [
			'type' => 'select',
		...
		if ($name == 'action_date') {
			...
			<div class="preserve-value">
				<span class="radio-list">
					...
					<?php if ($this->getValue('action_date_type') == 'fixed') {
						echo 'checked="checked"';
					} ?>
					>
					<?php echo print_widget($prefix.$name, ['type' => 'date'], $this->getValue('action_date')); ?>
				</span>
				<br />
				<span class="radio-list">
					<input type="radio" name="<?php echo $prefix; ?>action_date_type" value="relative"
					<?php if ($this->getValue('action_date_type') == 'relative') {
						echo 'checked="checked"';
					} ?>
					>
