			...
			default:
				if (strpos($fieldname, 'comps_') === 0) {
					...
					foreach ($this->getItems(FALSE, $compCatID) as $item) {
						...
						if (!$printableMode && (strlen($item['ccli_number'] ?? '') + strlen($item['comments'] ?? '') > 0) && $GLOBALS['user_system']->havePerm(PERM_VIEWSERVICE)) {
							// yuck, but oh well...
							ob_start();
							$dummy_comp = new Service_Component();
							$dummy_comp->setValue('ccli_number', $item['ccli_number']);
							$dummy_comp->printFieldValue('ccli_number');
							$ccli_code = ob_get_clean();
							...
							if ($GLOBALS['user_system']->havePerm(PERM_SERVICECOMPS)) {
								$line .= '<a title="Edit this component" href="'.BASE_PATH.'/?view=_edit_service_component&service_componentid='.$compid.'"><i class="icon-wrench"></i></a>';
							}
							$line .= '</td></tr>';
