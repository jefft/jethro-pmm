...
class Person extends DB_Object
{
	...
	private $_photo_data;
	private $_custom_values = [];
	private $_old_custom_values = [];
	...
	protected static function _getFields()
	{
		...
		$res = [
			...
			'gender' => [
				'type' => 'select',
				'options' => ['female' => 'Female', 'male' => 'Male', '' => 'Unknown'],
				'default' => 'female',
				'divider_before' => true,
			],
			'age_bracketid' => [
	...
	function printFieldValue($name, $value = null)
	{
		...
		switch ($name) {
			...
			case 'mobile_tel':
				...
				$links = ['<a href="tel:'.ents($value).'"><i class="icon-phone"></i> Call</a>'];
	...
	{
		$family_notes = $GLOBALS['system']->getDBObjectData('family_note', ['familyid' => $this->getValue('familyid')]);
		$person_notes = $GLOBALS['system']->getDBObjectData('person_note', ['personid' => $this->id]);
	...
	public function hasMemberAccount()
	{
		...
		return $row['gotpassword'] ? true : ($row['resetexpires'] ?: false);
	}
	...
	function saveAttendance($attendances, $groupid, $checkinid = null)
	{
		...
		$SQL = 'DELETE FROM attendance_record
				WHERE personid = '.(int) $this->id.'
				AND date IN ('.implode(',', array_map([$db, 'quote'], array_keys($attendances))).')
		...
		foreach ($attendances as $date => $present) {
			if ($present == '' || $present == '?' || $present == 'unknown') {
				continue;
			}
	...
	{
		$keys = ['first_name', 'last_name', 'email', 'mobile_tel'];
		foreach ($keys as $k) {
			if (isset($match_data[$k])) {
				$match_data[$k] = trim($match_data[$k]);
			}
			if (empty($match_data[$k])) {
				unset($match_data[$k]);
			}
		}
			...
			if ($cmp['last_name'] == $MATCH) {
				if ($cmp['first_name'] == $MATCH) {
					if (($cmp['mobile_tel'] != $DIFFERENT) && ($cmp['email'] != $DIFFERENT)) {
						...
						return [$top['id'] => true];
					} elseif (($cmp['mobile_tel'] = $MATCH) || ($cmp['email'] == $MATCH)) {
	...
	public function save($update_family = true)
	{
		...
		if ($update_family && $GLOBALS['user_system']->havePerm(PERM_EDITPERSON)) {
			...
			if (!empty($this->_old_values['status']) || !empty($this->_old_values['last_name'])) {
				...
				if (!empty($this->_old_values['status']) && in_array($this->getValue('status'), $archivedStatuses, true)) {
					...
					foreach ($members as $id => $details) {
						...
						if (!in_array($details['status'], $archivedStatuses, true)) {
				...
				if (in_array(array_get($this->_old_values, 'status'), $archivedStatuses, true) && (!in_array($this->getValue('status'), $archivedStatuses, true))) {
				...
				if (strlen($value)) {
					switch ($customFields[$fieldid]['type']) {
						...
						case 'select':
							...
							if ($idVal) {
								$optionVal = $value;
							} elseif (strlen($otherVal) && !empty($customFields[$fieldid]['params']['allow_other'])) {
	 ...
	 * @param string $plannedAbsenceDate (optional) - if specified, planned absences on this date should be displayed in results
	 */
	static function printMultipleFinder($name, $val = [], $plannedAbsenceDate = null)
	{
		$persons = empty($val) ? [] : $GLOBALS['system']->getDBObjectData('person', ['id' => $val]);
	...
	static function getCustomMergeData($personids, $formatted = true)
	{
		...
		$SQL = 'SELECT '.Custom_Field::getRawValueSQLExpr('v', 'f').' AS value, f.name, v.personid, v.fieldid, f.type
				FROM custom_field_value v
				JOIN custom_field f ON v.fieldid = f.id
				WHERE v.personid IN ('.implode(',', array_map([$db, 'quote'], $personids)).')';
	...
	function printForm($prefix = '', $fields = null)
	{
		...
		if (empty($fields) || in_array('custom', $fields, true)) {
			$customFields = self::getCustomFields();
			$dummyField = new Custom_Field();
			if ($customFields) {
				?>
				...
				foreach ($customFields as $fieldid => $fieldDetails) {
					...
					$values = $this->_custom_values[$fieldid] ?? [''];
	...
	function printFieldInterface($name, $prefix = '')
	{
		switch ($name) {
			...
			case 'familyid':
				...
				<br />
				<a href="<?php echo build_url(['view' => '_edit_family', 'familyid' => $this->values['familyid']]); ?>">
				<i class="icon-wrench"></i>Edit family details</a>
				&nbsp;
				<a href="<?php echo build_url(['view' => '_move_person_to_family', 'personid' => $this->id]); ?>">
				<i class="icon-random"></i>Move to different family</a>
				</div>
				<?php
				break;
			case 'congregationid':
				$stats = $GLOBALS['system']->getDBOBjectData('person_status', ['active' => 1]);
	...
	public function setFeedUUID()
	{
		...
		while ($others = $GLOBALS['system']->getDBObjectData('person', ['feed_uuid' => $uuid])) {
			$uuid = generate_random_string(60);
		}
	...
	public function fromCsvRow($row, $overwriteExistingValues = true)
	{
		...
		if (empty($this->id) && !isset($row['gender'])) {
			$row['gender'] = 'Unknown';
		} // only apply the female default via the GUI, not import
		foreach ($row as $k => $v) {
	...
	public function archiveAndClean()
	{
		...
		$notes = $GLOBALS['system']->getDBObjectData('person_note', ['personid' => $this->id]);
		...
		foreach ($members as $id => $details) {
			...
			if (!in_array($details['status'], Person_Status::getArchivedIDs(), true)) {
				$found_live_member = true;
				break;
			}
		}
		if (!$found_live_member) {
			if ($family->archiveAndClean()) {
				$res = 2;
			}
		}
