...
class Action_Plan extends DB_Object
{
	...
	protected static function _getFields()
	{
		return [
			...
			'default_on_create_family' => [
				'type' => 'select',
				'options' => ['No', 'Yes'],
				'default' => 0,
			],
			'default_on_add_person' => [
				'type' => 'select',
				'options' => ['No', 'Yes'],
				'default' => 0,
			],
	...
	function printForm($prefix = '', $fields = null)
	{
		...
		if (empty($notes)) {
			$notes = [[]]; // 1 blank note to start us off
		}
		?>
		...
		?>
				...
				<tr>
					...
					<td>
						...
						<?php
		$groups = array_get($actions, 'groups', []);
		if (empty($groups)) {
			$groups = [0];
		}
		$mstatuses = array_get($actions, 'group_membership_statuses', []);
		$group_mark_present = array_get($actions, 'group_mark_present', []);
		...
		?>
				...
				<tr>
					...
					<td>
						...
						<?php
		Person_Group::printMultiChooser('groups_remove', array_get($actions, 'groups_remove', []), [], false);
		?>
						<br />
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<table class="fields">
						<?php
		$fields = array_get($actions, 'fields', []);
		$dummy = new Person();
		foreach (['congregationid', 'status', 'age_bracketid'] as $field) {
			$value = '';
			$addToExisting = false;
			...
			echo '<tr><td>';
			print_widget('fields_enabled['.$field.']', ['type' => 'checkbox'], strlen($value));
		...
		foreach ($customFields as $fieldid => $fieldDetails) {
			...
			echo '<tr><td>';
			print_widget('fields_enabled[custom_'.$fieldid.']', ['type' => 'checkbox'], strlen($value));
			...
			if ($fieldDetails['type'] == 'date') {
				...
				$dateVal = $noteVal = '';
				if (strlen($value)) {
					[$dateVal, $noteVal] = explode('===', $value);
				}
				echo 'to the reference date, with note ';
				print_widget('custom_'.$fieldid.'_note', ['type' => 'text'], $noteVal);
			} else {
		...
		?>
				...
				<tr>
					...
					<td>
						...
						<label class="checkbox">
							<input type="checkbox" id="mark_present" name="mark_present" value="1" <?php if (array_get($actions, 'attendance')) {
				...
				<tr>
					...
					<td>
						...
						<label class="checkbox">
							<input type="checkbox" id="default_on_create_family" name="default_on_create_family" value="1" <?php if ($this->getValue('default_on_create_family')) {
								echo 'checked="checked"';
							} ?>>
						...
						<label class="checkbox">
							<input type="checkbox" id="default_on_add_person" name="default_on_add_person" value="1" <?php if ($this->getValue('default_on_add_person')) {
								echo 'checked="checked"';
							} ?>>
		...
		?>
						...
						<div id="agebrackets" <?php if (empty($abs)) {
	...
	function processForm($prefix = '', $fields = null)
	{
		...
		$addValue = array_get($_POST, 'fields_addvalue', []);
		foreach (array_get($_POST, 'fields_enabled', []) as $k => $v) {
		...
		$this->_tmp['abs'] = !empty($_REQUEST['age_brackets_all']) ? [] : $_REQUEST['age_bracketids'];
	}
	...
	function create()
	{
		...
		$res = parent::create();
		if ($res) {
			$this->saveAgeBracketRestrictions();
		}
	...
	{
		$res = [];
		$plans = $GLOBALS['system']->getDBObjectData('action_plan', [], 'OR', 'name');
		foreach ($plans as $id => $plan) {
			$selected = false;
			if (is_array($value_or_context)) {
				$selected = in_array($id, $value_or_context, true);
			} elseif ($value_or_context == 'create_family') {
				$selected = $plan['default_on_create_family'];
			} elseif ($value_or_context == 'add_person') {
	...
	function execute($subject_type, $subject_id, $reference_date)
	{
		...
		} elseif ($subject_type == 'person') {
			$personids = [$subject_id];
		} else {
		...
		$actions = $this->getValue('actions');
		$membershipStatuses = array_get($actions, 'group_membership_statuses', []);
		$groupMarkPresent = array_get($actions, 'group_mark_present', []);
		foreach (array_get($actions, 'groups', []) as $i => $groupid) {
			$group = $GLOBALS['system']->getDBObject('person_group', $groupid);
			...
			if (!empty($groupMarkPresent[$i])) {
				...
				foreach ($personids as $personid) {
					...
					$person->saveAttendance([$date => 1], $groupid);
				}
			}
		}
		foreach (array_get($actions, 'groups_remove', []) as $groupid) {
			$group = $GLOBALS['system']->getDBObject('person_group', $groupid);
			if ($group) {
		...
		foreach (array_get($actions, 'notes', []) as $notedata) {
			$note = new $note_type();
		...
		if (array_get($actions, 'attendance')) {
			foreach ($personids as $personid) {
				...
				if ($congID) {
					...
					$person->saveAttendance([$date => 1], null);
				}
			}
		}
