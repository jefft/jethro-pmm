...
class Roster_Role extends db_object
{
	...
	function __construct($id = null)
	{
		...
		} elseif (empty($id) && !empty($_SESSION['role_congregationid'])) {
	...
	{
		return ['roster_role.volunteer_group' => '_person_group(id)'];
	}
	...
	function printFieldInterface($name, $prefix = '')
	{
		if (($name == 'volunteer_group') && (empty($this->id) || $this->haveLock())) {
			...
			Person_Group::printChooser($prefix.$name, $value, [], null, '(None)');
		} else {
			if ($name == 'active') {
	...
	function printFieldValue($name, $value = null)
	{
		switch ($name) {
			case 'details':
				echo '<a class="pull-right" target="publicrole" href="'.BASE_PATH.'/public/?view=display_role_description&role='.$this->id.'"><i class="icon-share"></i>View in public area</a>';
	...
	private function _printChooserOption($vid, $name, $selectedid, &$absentees)
	{
		...
		if ($vid == $selectedid) {
			...
			if (in_array($vid, $absentees, true)) {
				$note = ' !! ABSENT !!';
			}
		} else {
			if (in_array($vid, $absentees, true)) {
	 ...
	 */
	function printChooser($date, $currentval = [''], $absentees = [])
	{
			...
			if ($this->getValue('assign_multiple')) {
				...
				foreach ($currentval as $id) {
					...
					<select name="assignees[<?php echo $this->id; ?>][<?php echo $date; ?>][]">
						<option value=""></option>
					<?php
					if (!empty($id) && !isset($volunteers[$id])) {
						$this->_printUnlistedAlloceeOption($id);
					}
				...
				<select name="assignees[<?php echo $this->id; ?>][<?php echo $date; ?>]">
					<option value=""></option>
				<?php
				if (!empty($currentID) && !isset($volunteers[$currentID])) {
					$this->_printUnlistedAlloceeOption($currentID);
				}
