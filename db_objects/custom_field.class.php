...
class Custom_Field extends db_object
{
	...
	public function getOptions()
	{
		...
		if (!isset($this->_tmp['options'])) {
			$this->_tmp['options'] = [];
			$opts = $GLOBALS['system']->getDBObjectData('custom_field_option', ['fieldid' => $this->id], 'OR', 'rank');
	...
	function printFieldInterface($fieldname, $prefix = '')
	{
		switch ($fieldname) {
			case 'heading_before_toggle':
				...
				print_widget(
					$prefix.$fieldname,
					['type' => 'checkbox', 'attrs' => ['title' => $title]],
			...
			case 'searchable':
				print_widget(
					$prefix.$fieldname,
					['type' => 'checkbox', 'attrs' => ['title' => $this->fields[$fieldname]['title']]],
			...
			case 'params':
				...
				if ($this->id) {
					...
					call_user_func(['Custom_Field', $fn], $prefix, $params);
				} else {
	...
	public static function printParamsDate($prefix, $params)
	{
		...
		<label class="allownote">
			<?php print_widget($prefix.'allow_note', ['type' => 'checkbox'], array_get($params, 'allow_note')); ?>
			Allow note
		</label>
		<label class="allownote">
			<?php print_widget($prefix.'allow_blank_year', ['type' => 'checkbox'], array_get($params, 'allow_blank_year')); ?>
			Allow blank year
		</label>
		<?php
	}
	...
	public static function printParamsText($prefix, $params)
	{
		...
		Regex:
		<?php
		print_widget($prefix.'regex',
			[
				'type' => 'text',
				'attrs' => ['placeholder' => 'Optional, eg ^[0-9]+$']],
			array_get($params, 'regex'),
		);
		?>
		</label>
		<?php
	...
	public static function printParamsLink($prefix, $params)
	{
		...
		Template:
		<?php
		print_widget($prefix.'template', ['type' => 'text', 'attrs' => ['placeholder' => '(Optional)']], array_get($params, 'template'));
		?>
		</label>
	...
	public static function printParamsSelect($prefix, $params)
	{
		...
		foreach ($options as $optionID => $optionValue) {
			...
			?>
						...
						<td>
							<?php print_widget($prefix.'option_values[]', ['type' => 'text'], $optionValue); ?>
						</td>
						<td class="center">
							<?php
			if ($optionID !== '') {
			...
			<tfoot>
				<tr>
					<td colspan="2">
						<label class="radio">
							<?php
					print_widget($prefix.'allow_other', ['type' => 'checkbox'], array_get($params, 'allow_other', false));
		echo _('Allow "other"');
		?>
						</label>
			</tfoot>
		...
		switch ($fieldname) {
			...
			case 'params':
				...
				if (!empty($_REQUEST[$prefix.'option_ids'])) {
					$newOptions = $updateOptions = $deleteOptions = [];
					foreach ($_REQUEST[$prefix.'option_ids'] as $rank => $optionID) {
						$value = $_REQUEST[$prefix.'option_values'][$rank];
						if ($optionID) {
							// Update
							if (in_array($optionID, array_get($_REQUEST, $prefix.'options_delete', []), true)) {
	...
	private function getWidgetParams()
	{
		...
		if ($this->getValue('type') == 'select') {
			$options = $GLOBALS['system']->getDBObjectData('custom_field_option', ['fieldid' => $this->id], 'OR', 'rank');
		...
		if (!empty($this->values['params']['allow_blank_year'])) {
			$params['allow_blank_year'] = $this->values['params']['allow_blank_year'];
		}
		if (!empty($this->values['params']['regex'])) {
			$params['regex'] = $this->values['params']['regex'];
		}
	 ...
	 */
	public function printWidget($value, $extraParams = [], $prefix = '')
	{
		...
		if (($this->getValue('type') == 'select') && strlen($value) && !empty($this->values['params']['allow_other'])) {
			if (!isset($widgetParams['options'][$value])) {
				$otherValue = $value;
				if (str_starts_with($otherValue, '0 ')) {
					$otherValue = substr($otherValue, 2);
				}
				$value = 'other';
			}
		}
		...
		if (($this->getValue('type') == 'date') && !empty($this->values['params']['allow_note'])) {
			...
			print_widget($prefix.'custom_'.$this->id.'_note[]', ['type' => 'text', 'placeholder' => '(Note)'], $note);
		}
		if (($this->getValue('type') == 'select') && !empty($this->values['params']['allow_other'])) {
			print_widget($prefix.'custom_'.$this->id.'_other[]', ['type' => 'text', 'class' => 'select-other'], $otherValue);
		}
	...
	public function processWidget($prefix = '')
	{
		...
		if (($this->getValue('type') == 'date') && !empty($this->values['params']['allow_note'])) {
			$notes = process_widget($prefix.'custom_'.$this->id.'_note', ['type' => 'text'], null, true);
	...
	{
		if (is_array($val)) {
			return implode(', ', array_map([$this, 'formatValue'], $val));
		}
		if (!strlen($val)) {
			return '';
		}
	...
	public function printFormattedValue($val)
	{
		if ($this->getValue('type') == 'link') {
			...
			if (in_array($this->getValue('type'), ['date', 'select'], true)) {
				$res = nbsp($res);
			}
			echo $res;
		}
