...
class View__Edit_Me extends View
{
	...
	private $person_fields = ['gender', 'age_bracketid', 'email', 'mobile_tel', 'work_tel', 'photo'];
	...
	{
		if ($this->family) {
			return 'Editing '.$this->family->getValue('family_name').' Family';
		}
	}
	...
	function canEditFamily()
	{
		...
		return
			in_array($GLOBALS['user_system']->getCurrentMember('age_bracketid'), Age_Bracket::getAdults(), true)
			|| !$this->hasAdult
		;
	}
	...
	function processView()
	{
		...
		foreach ($this->family->getMemberData() as $id => $member) {
			...
			if ($p) {
				$this->persons[$id] = $p;
				if (in_array($p->getValue('age_bracketid'), Age_Bracket::getAdults(), true)) {
					$this->hasAdult = true;
				}
			} else {
		...
		if (array_get($_POST, 'family_details_submitted')) {
			...
			$fields = ['gender', 'age_bracket', 'email', 'mobile_tel', 'work_tel'];
			foreach ($this->persons as $person) {
				...
				} elseif ($this->canEditFamily() || $this->isMe($person)) {
	...
	function printView()
	{
		...
		if (!$ok) {
			...
			<?php
			if ($this->canEditFamily()) {
				$this->family->printForm('family', ['address_street', 'address_suburb', 'address_postcode', 'home_tel', 'photo']);
			} else {
			...
			foreach ($this->persons as $person) {
				...
				$sm = new Staff_Member($person->id);
				if ($sm && $sm->requires2FA()) {
					echo '<p><i>This person has a control centre account, so their details can only be edited via the <a href="'.BASE_PATH.'?view=persons&personid='.(int) $person->id.'">control centre</a></i></p>';
				} elseif ($this->canEditFamily() || $this->isMe($person)) {
