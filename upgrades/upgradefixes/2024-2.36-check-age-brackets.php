...
define('JETHRO_ROOT', dirname(__DIR__, 2));
...
if (!empty($groupedchanges)) {
	if (array_key_exists(MULTIPLE_PERSONS, $groupedchanges)) {
		...
		echo 'Warning: '.count($allaffected).' people have been incorrectly turned into Adults by bug https://github.com/tbar0970/jethro-pmm/issues/1086:'.\PHP_EOL;
		...
		$allaffected = array_unique(array_merge(...$allaffected));
		echo 'Warning: '.count($allaffected).' people MAY have been incorrectly turned into Adults by bug https://github.com/tbar0970/jethro-pmm/issues/1086:'.\PHP_EOL;
	}
	foreach ($allaffected as $affected) {
		echo $affected.\PHP_EOL;
	}
	echo 'Please log in to Jethro to fix this - '.build_url(['view' => '_fix_age_brackets']).\PHP_EOL;
}
