...
.modal .day-box, .modal .year-box {
	/* The ex widths above were tuned as *content* widths. Inside modals,
	 * .modal input's box-sizing: border-box (see zlvqumxl / #1521) would
	 * subtract padding and border from them, clipping the digits. These
	 * inputs are far narrower than any modal body, so max-width: 100% can
	 * never bite, making content-box safe here. */
...
/* Required/optional marker in the SMS "not configured" constants table. */
.sms-constant-required {
	color: #b94a48;
}
.sms-constant-optional {
	color: #999;
}
