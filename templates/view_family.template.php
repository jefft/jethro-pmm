...
if ($GLOBALS['user_system']->havePerm(PERM_VIEWNOTE)) {
	$notes = $GLOBALS['system']->getDBObjectData('family_note', ['familyid' => $family->id], 'OR', 'created');
}
...
if (!$accordion) {
	...
	if ($GLOBALS['user_system']->havePerm(PERM_VIEWNOTE)) {
		...
		$notes = $GLOBALS['system']->getDBObjectData('family_note', ['familyid' => $family->id], 'OR', 'created '.$order);
		?>
...
foreach ($family->getMemberData() as $personid => $person) {
	...
	$archivedClass = in_array($person['status'], Person_Status::getArchivedIDs(), true) ? 'archived' : '';
	?>
...
if ($GLOBALS['user_system']->havePerm(PERM_VIEWNOTE)) {
	...
	if ($GLOBALS['user_system']->havePerm(PERM_EDITNOTE)) {
		...
		if (count($members) > 1) {
			...
			?>
				<a href="?view=_add_note_to_person&personid=<?php $memberarray = array_keys($members);
			echo reset($memberarray); ?>"><?php echo _('Add Person Note'); ?></a>
				<?php
		}
		?>
		</div>
