...
<div class="form-horizontal bulk-actions">
	...
	if (version_compare(PHP_VERSION, '5.2', '>=') && !SizeDetector::isNarrow()) {
		...
		<div class="bulk-action well" id="document-merge">
			<div class="control-group">
				...
				if (!empty($templates)) {
					...
					<p class="help-inline">
						<a target="roster-merge-help" class="med-newwin" href="<?php echo BASE_PATH; ?>index.php?call=document_merge_help"><i class="icon-help"></i>Help and examples</a>
						&nbsp;
						<button type="submit" class="btn btn-mini muted" name="preview_keywords" onclick="$('input[name=source_document]').removeClass('compulsory')" data-set-form-target="_blank" data-set-form-action="<?php echo BASE_PATH; ?>index.php?call=document_merge">Preview all tags</button>
					</p>
					<?php
				} else {
					?>
		...
		?>
			...
			<div class="control-group">
				<div class="controls">
					<input type="submit" class="btn " value="Go" data-set-form-action="<?php echo BASE_PATH; ?>index.php?call=document_merge" />
				</div>
			</div>
