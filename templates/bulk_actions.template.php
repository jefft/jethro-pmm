...
$plan_chooser = Action_Plan::getMultiChooser('planid', []);
if ($plan_chooser) {
	?>
...
?>
	...
	if ($GLOBALS['user_system']->havePerm(PERM_EDITGROUP)) {
		...
		<span class="bulk-action" id="remove-from-group">
			<input type="submit" class="btn " value="Go" data-set-form-action="<?php echo BASE_PATH; ?>/?view=_edit_group&action=remove_members&groupid=<?php echo $groupid; ?>" />
		</span>
		...
		<div class="bulk-action well" id="update-field">
			...
			<?php
			$dummy = new Person();
		foreach (['status', 'age_bracketid'] as $field) {
		...
		foreach ($GLOBALS['system']->getDBObjectdata('congregation', []) as $cid => $cong) {
		...
		$addParams = [
			'type' => 'select',
			'options' => ['Replacing existing values ', 'Adding to existing values'],
		];
		...
		?>
			...
			<input type="submit" class="btn" onclick="return confirm('Are you sure you want to bulk-update these persons?')" value="Go" data-set-form-action="<?php echo BASE_PATH; ?>/?view=_persons_bulk_update&backto=<?php echo urlencode(http_build_query($_GET)); ?>" />
		</div>
		...
		foreach ($verbs as $verb) {
			...
			?>
					<input type="submit" class="btn " value="Go" data-require-fields="#<?php echo $verb; ?>_existing_group [name=groupid]" data-set-form-action="<?php echo BASE_PATH; ?>/?view=_edit_group&action=add_members" />
				</div>
				</p>
			...
			?>
							...
							<input type="submit" class="btn " value="Go" data-require-fields="<?php echo $verb; ?>_new_group [name=name]" data-set-form-action="<?php echo BASE_PATH; ?>/?view=groups__add" />
						</td>
					</tr>
				</table>
				</p>
...
if ($GLOBALS['user_system']->havePerm(PERM_EDITNOTE)) {
	...
	?>
			<div class="control-group">
				<div class="controls">
					<input type="submit" name="new_note_submitted" class="btn " value="Go" data-set-form-action="<?php echo BASE_PATH; ?>/?view=_add_note_to_person" />
				</div>
			</div>
		</div>
		<?php
...
?>
		<div class="bulk-action well" id="email">
			...
			<input type="submit" class="btn " value="Go" data-set-form-target="hidden" data-set-form-action="<?php echo BASE_PATH; ?>/?call=email" />
		</div>
	<?php
if (SMS_Sender::canSend()) {
	?>
			...
			<div class="control-group">
				...
				<div class="controls">
				  <label class="checkbox">
					<input type="checkbox" name="saveasnote" <?php if (ifdef('SMS_SAVE_TO_NOTE_BY_DEFAULT')) {
...
?>
		<div class="bulk-action well" id="export">
			...
			<input type="submit" class="btn " value="CSV" data-set-form-action="<?php echo BASE_PATH; ?>/?call=csv&merge_type=person" />
			<input type="submit" class="btn" value="vCard" data-set-form-action="<?php echo BASE_PATH; ?>/?call=vcf" />
			&nbsp;&nbsp;Download Family data:
			<input type="submit" class="btn " value="CSV" data-set-form-action="<?php echo BASE_PATH; ?>/?call=csv&merge_type=family" />
		</div>
    <?php
if (version_compare(\PHP_VERSION, '5.2', '>=') && !SizeDetector::isNarrow()) {
	...
	?>
		<div class="bulk-action well" id="document-merge">
			<div class="control-group">
				...
				<?php
			if (!empty($templates)) {
				$tOptions = ['' => '', '__NEW__' => 'Upload a new file...'];
				foreach ($templates as $t) {
					$tOptions[basename($t)] = basename($t);
				}
				print_widget('source_doc_select', ['type' => 'select', 'options' => $tOptions, 'class' => 'merge-template'], '');
				?>
					...
					<p class="help-inline">
						<a target="roster-merge-help" class="med-newwin" href="<?php echo BASE_PATH; ?>/?call=document_merge_help"><i class="icon-help"></i>Help and examples</a>
						&nbsp;
						<button type="submit" class="btn btn-mini muted" name="preview_keywords" onclick="$('input[name=source_document]').removeClass('compulsory')" data-set-form-target="_blank" data-set-form-action="<?php echo BASE_PATH; ?>/?call=document_merge">Preview all tags</button>
					</p>
					<?php
			} else {
				?>
	...
	?>
			...
			<?php if (!empty($_REQUEST['queryid'])) {
				print_hidden_field('queryid', $_REQUEST['queryid']);
			} ?>
			<div class="control-group">
				<div class="controls">
					<input type="submit" class="btn " value="Go" data-set-form-action="<?php echo BASE_PATH; ?>/?call=document_merge" />
				</div>
			</div>
...
?>
		<div class="bulk-action well" id="envelopes">
			...
			<input type="submit" class="btn " value="Go" data-set-form-target="envelope" data-set-form-action="<?php echo BASE_PATH; ?>/?call=envelopes" />
		</div>
	<?php
if ($plan_chooser) {
	?>
		...
		<p><?php echo _('Reference date for plans:'); ?> <?php print_widget('plan_reference_date', ['type' => 'date'], null); ?>
		&nbsp;
		<input type="submit" value="Go" data-set-form-action="<?php echo BASE_PATH; ?>/?view=_execute_plans" /></p>
		</div>
		<?php
}
